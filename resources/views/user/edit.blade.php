            ...
            <form action="{{ route('user.update', ['user' => $user]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    ...
                    <div class="row mb-4 col-12 ">
                        <label for="email" class="col-sm-3 col-form-label text-end px-4">Email: </label>
                        <div class="col-sm-6">
                            <input type="email" name="email" placeholder="Dirección de correo eléctronico"
                                id="email" class="form-control" value="{{ old('email', $user->email) }}">
                        </div>
                        <div class="col-sm-3 ">
                            @error('email')
                                <small class="text-danger">{{ '*' . $message }}</small>
                        ...
                        <div class="col-sm-6">
                            <select name="role" id="role" class="form-select">
                                ...
                                @foreach ($roles as $item)
                                    @if (in_array($user->id, $item->permissions->pluck('id')->toArray()))
